...
st.title("🎨 Textured Paint Recolor (Debug WB Gains + Overall Impression Match)")

# =========================
# Constants (no sliders)
# =========================
GLOBAL_BRIGHTNESS_BIAS = 0.94  # adjust if needed: 0.92 darker, 0.96 lighter
...
MAX_ZIP_MB = 180  # approximate safety limit

# =========================
# WB + Debug
# =========================
def compute_grayworld_gains(img_rgb: np.ndarray) -> np.ndarray:
    """Compute gray-world gains (target is mean of channel means)."""
...
def white_balance_preserve_luma(img_rgb: np.ndarray) -> np.ndarray:
    """
    Gray-world WB with overall luminance preserved (prevents global brightening).
    """
    img = img_rgb.astype(np.float32)
...
def recolor_preserve_texture_with_L_match(img_rgb: np.ndarray, target_rgb: Tuple[int, int, int]) -> np.ndarray:
    ...
    LAB recolor:
      - preserve texture (L)
      - soft chroma mix
      - remap L to target L (helps light colors)
    """
...
def match_overall_impression_to_target(recolored_u8: np.ndarray, target_rgb: Tuple[int, int, int]) -> np.ndarray:
    ...
    - sRGB luma proxy
    - highlight-aware anchor (70th percentile within 25–85%)
    - global bias
    """
...
img = np.array(Image.open(io.BytesIO(img_bytes)).convert("RGB"))
    ...
    # --- Debug: show WB means + gains ---
    means, gains = compute_grayworld_gains(img)
    st.subheader("White Balance Debug")
    st.write("Mean RGB (R,G,B) before WB:", means)
    st.write("Gray-world gains (R,G,B):", gains)

    wb = white_balance_preserve_luma(img)

    st.subheader("Working Image (White Balance with Luma Preserved)")
    st.image(wb, caption="May look identical if gains are near 1.0 (image already neutral).", use_column_width=True)

    # Optional: show amplified diff map (helps confirm subtle WB)
    with st.expander("Show WB difference map (amplified)"):
        diff = wb.astype(np.int16) - img.astype(np.int16)
        diff_vis = np.clip(diff * 6 + 128, 0, 255).astype(np.uint8)
        st.image(diff_vis, caption="WB delta amplified ×6 (128 = no change)", use_column_width=True)
    ...
    with tab_batch:
        ...
        st.caption("Build one ZIP at a time to avoid crashes. ZIP size can get very large.")
